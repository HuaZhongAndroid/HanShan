 ...
 */
public class CircleProgressBarRefush extends ImageView {
    ...
    public CircleProgressBarRefush(Context context) {
        super(context);
        init(context, null, 0);

    }

    public CircleProgressBarRefush(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);

    }

    public CircleProgressBarRefush(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.CircleProgressBarRefush, defStyleAttr, 0);

        final float density = getContext().getResources().getDisplayMetrics().density;

        mBackGroundColor = a.getColor(
                R.styleable.CircleProgressBarRefush_background_color, DEFAULT_CIRCLE_BG_LIGHT);

        mProgressColor = a.getColor(
                R.styleable.CircleProgressBarRefush_progress_color, DEFAULT_CIRCLE_COLOR);
        mColors = new int[]{mProgressColor};

        mInnerRadius = a.getDimensionPixelOffset(
                R.styleable.CircleProgressBarRefush_inner_radius, -1);

        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.CircleProgressBarRefush_progress_stoke_width, (int) (STROKE_WIDTH_LARGE * density));
        mArrowWidth = a.getDimensionPixelOffset(
                R.styleable.CircleProgressBarRefush_arrow_width, -1);
        mArrowHeight = a.getDimensionPixelOffset(
                R.styleable.CircleProgressBarRefush_arrow_height, -1);
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.CircleProgressBarRefush_progress_text_size, (int) (DEFAULT_TEXT_SIZE * density));
        mTextColor = a.getColor(
                R.styleable.CircleProgressBarRefush_progress_text_color, Color.BLACK);

        mShowArrow = a.getBoolean(R.styleable.CircleProgressBarRefush_show_arrow, true);
        mCircleBackgroundEnabled = a.getBoolean(R.styleable.CircleProgressBarRefush_enable_circle_background, true);


        mProgress = a.getInt(R.styleable.CircleProgressBarRefush_progress, 0);
        mMax = a.getInt(R.styleable.CircleProgressBarRefush_max, 100);
        int textVisible = a.getInt(R.styleable.CircleProgressBarRefush_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
...
//        }
    ...
    private class OvalShadow extends OvalShape {
        ...
        public void draw(Canvas canvas, Paint paint) {
            final int viewWidth = CircleProgressBarRefush.this.getWidth();
            final int viewHeight = CircleProgressBarRefush.this.getHeight();
