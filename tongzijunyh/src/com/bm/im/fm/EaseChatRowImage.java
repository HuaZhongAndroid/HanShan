...
import com.bm.im.ac.MyEaseShowBigImageActivity;
...
public class EaseChatRowImage extends EaseChatRowFile{
    ...
    protected void onBubbleClick() {
        Intent intent = new Intent(context, MyEaseShowBigImageActivity.class);
        
        ArrayList<ImageMessageBody> imgBodyList = new ArrayList<ImageMessageBody>();
        for(int i=0; i<adapter.getCount(); i++)
        {
        	EMMessage msg = (EMMessage)adapter.getItem(i);
        	
        	if(msg != null && msg.getType() == EMMessage.Type.IMAGE)
        	{
        		imgBodyList.add((ImageMessageBody)msg.getBody());
        	}
        }
        intent.putExtra("imgBodyList", imgBodyList);
        intent.putExtra("position", imgBodyList.indexOf(imgBody));
