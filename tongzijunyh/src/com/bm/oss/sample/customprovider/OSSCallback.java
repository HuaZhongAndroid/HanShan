package com.bm.oss.sample.customprovider;
...
//    private TextView infoView;
    ...
    /* 必须在UI线程中初始化handler */
    public OSSCallback(Activity activity) {
...
//        this.infoView = infoView;
        ...
        handler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message inputMessage) {
                String info;
                onCallback(inputMessage);
                switch (inputMessage.what) {
                    case UPLOAD_OK:
//                        new AlertDialog.Builder(OSSCallback.this.activity).setTitle("上传成功").setMessage("upload to OSS OK!").show();
                        break;
                    case UPLOAD_FAIL:
                        info = (String) inputMessage.obj;
//                        new AlertDialog.Builder(OSSCallback.this.activity).setTitle("上传失败").setMessage(info).show();
                        break;
                    case DOWNLOAD_OK:
//                        new AlertDialog.Builder(OSSCallback.this.activity).setTitle("下载成功").setMessage("download from OSS OK!").show();
                        break;
                    case SETTING_OK:
//                        new AlertDialog.Builder(OSSCallback.this.activity).setTitle("设置成功").setMessage("设置域名信息成功,现在<选择图片>, 然后上传图片").show();
                        break;
                    case DOWNLOAD_FAIL:
                        info = (String) inputMessage.obj;
//                        new AlertDialog.Builder(OSSCallback.this.activity).setTitle("下载失败").setMessage(info).show();
                        break;
                    case UPDATE_PROGRESS:
//                        OSSCallback.this.bar.setProgress(inputMessage.arg1);
                        //Log.d("UpdateProgress", String.valueOf(inputMessage.arg1));
                        break;
                    case DISPLAY_IMAGE:
                        Bitmap bm = (Bitmap) inputMessage.obj;
//                        OSSCallback.this.imageView.setImageBitmap(bm);
                        break;
                    case DISPLAY_INFO:
                        info = (String) inputMessage.obj;
//                        OSSCallback.this.infoView.setText(info);
...
//        Log.d("Width", String.valueOf(imageView.getWidth()));
    ...
    protected abstract void onCallback(Message inputMessage);
