 ...
 *
 */
public class PayInfoAc2 extends BaseActivity implements OnClickListener {
    ...
    private CourseBao bao; //课程包

    private String payPwd="",strPageTag="";//支付密码
    //	Order order; //微信支付用的
    public static PayInfoAc2 intance;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        contentView(R.layout.ac_payinfo2);
        mContext = this;
        setTitleName("支付订单");
        intance = this;
        quan = null;
    ...
    private void defYouhui()
    {
        ...
        if(course != null) {
            map.put("goodsType", course.goodsType);
            map.put("goodsPrice", course.goodsPrice);
        }else if(bao != null){
            map.put("goodsType", bao.type);
            map.put("goodsPrice", bao.money);
        }
    ...
    public void init() {
        ...
        tv_jiaotongfei = this.findTextViewById(R.id.tv_jiaotongfei);
        v_jiaotongfei = this.findViewById(R.id.v_jiaotongfei);


        bao = (CourseBao) getIntent().getSerializableExtra("bao");
        if(bao != null)
        {
            ((View)tv_babyname.getParent()).setVisibility(View.GONE);
            if(bao.storeName != null)
                tv_address.setText(getNullData(bao.storeName));
            else
                ((View)tv_address.getParent()).setVisibility(View.GONE);
            tv_time.setText(bao.groupDate);//时间
            findTextViewById(R.id.tv_time_t).setText("有效期限：");
            tv_course_name.setText(getNullData(bao.name));//课程名称
            tv_course_count = findTextViewById(R.id.tv_course_count);
            tv_course_count.setVisibility(View.VISIBLE);
            tv_course_count.setText("(内含"+bao.courseNum+"节课程)");
        }
        else
        {
            if(child != null)
                tv_babyname.setText(child.realName);
            else
                ((View)tv_babyname.getParent()).setVisibility(View.GONE);

            if(storelist != null)
                tv_address.setText(getNullData(storelist.storeName));
            else
                ((View)tv_address.getParent()).setVisibility(View.GONE);

            tv_time.setText(course.goodsTime);//时间
            tv_course_name.setText(getNullData(course.goodsName));//课程名称
        }
...
//				dialogToast("余额支付密码未设置，请到设置界面先设置支付密码");
                UtilDialog.dialogTwoBtnContentTtile(this, "余额支付密码未设置，请先设置支付密码", "取消","前往","提示",new Handler(){
                    public void handleMessage(android.os.Message msg) {
                        switch(msg.what){
                            case 1:
                                Intent intent = new Intent(PayInfoAc2.this,PwdSetAc.class);
...
//			}
    ...
    public void getPay(){
        HashMap<String, String> map = new HashMap<String, String>();
        if(course!= null)
            map.put("goodsId", course.goodsId);//课程ID
        else if(bao != null)
            map.put("goodsId", bao.pkid);//课程包ID
        if(null != App.getInstance().getUser()){
        ...
        if (payType == 1) {
            UserManager.getInstance().payAli(this, map, new ServiceCallback<CommonResult<AliOrder>>() {
                ...
                public void done(int what, CommonResult<AliOrder> obj) {
                    ...
                    if(null!=obj.data){
                        aliOrder=obj.data;
                        AlipayUtil.pay(aliOrder.payinfo, PayInfoAc2.this,strPageTag);
                    }
                }
            });
